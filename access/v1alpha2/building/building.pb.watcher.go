...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *building.Building_FieldMask
	OrderBy   *building.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *building.ParentReference
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[building.Name]*building.Building
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*building.BuildingChange
}

func NewWatcher(client building_client.BuildingServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &building.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &building.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "building-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[building.Name]*building.Building{}
...
type WatcherEventChange struct {
	pre, post *building.Building
	name      *building.Name
}

func NewAddWatcherEventChange(resource *building.Building) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *building.Building) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *building.Building) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *building.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *building.Building {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *building.Building {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *building.Building {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *building.Building {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[building.Name]*building.Building, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		building := currentChange.GetCurrent()
		byName[*building.GetName()] = building
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			building := change.GetCurrent()
			byName[*building.GetName()] = building
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, building := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(building))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[building.Name]*building.Building{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *building.Building
		switch item := change.ChangeType.(type) {
		case *building.BuildingChange_Added_:
			currentItem = item.Added.GetBuilding()
		case *building.BuildingChange_Current_:
			currentItem = item.Current.GetBuilding()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*building.BuildingChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *building.BuildingChange_Added_:
			newItem := item.Added.GetBuilding()
		...
		case *building.BuildingChange_Modified_:
			updatedItem := item.Modified.GetBuilding()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*building.BuildingChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetBuildingName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(building.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*building.Building_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*building.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(building_client.NewBuildingServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(building.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*building.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*building.ParentReference)
		}
		return params
	})
}
