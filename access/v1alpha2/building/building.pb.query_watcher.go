...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*building.BuildingChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return building.BuildingChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client building_client.BuildingServiceClient,
...
func NewQueryWatcherWithIChan(id int, client building_client.BuildingServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "building-query-watcher").
	...
	for {
		stream, err := qw.client.WatchBuildings(ctx, &building_client.WatchBuildingsRequest{
		...
		if err != nil {
			...
			pending := make([]*building.BuildingChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetBuildingChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetBuildingChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(building.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*building.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*building.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*building.Filter)
		}
		return NewQueryWatcherWithIChan(id, building_client.NewBuildingServiceClient(cc), cfg, ch)
	})
}
