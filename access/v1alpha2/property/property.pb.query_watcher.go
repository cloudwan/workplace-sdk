...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*property.PropertyChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return property.PropertyChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client property_client.PropertyServiceClient,
...
func NewQueryWatcherWithIChan(id int, client property_client.PropertyServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "property-query-watcher").
	...
	for {
		stream, err := qw.client.WatchProperties(ctx, &property_client.WatchPropertiesRequest{
		...
		if err != nil {
			...
			pending := make([]*property.PropertyChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetPropertyChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetPropertyChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(property.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*property.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*property.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*property.Filter)
		}
		return NewQueryWatcherWithIChan(id, property_client.NewPropertyServiceClient(cc), cfg, ch)
	})
}
