...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *property.Property_FieldMask
	OrderBy   *property.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *property.ParentReference
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[property.Name]*property.Property
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*property.PropertyChange
}

func NewWatcher(client property_client.PropertyServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &property.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &property.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "property-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[property.Name]*property.Property{}
...
type WatcherEventChange struct {
	pre, post *property.Property
	name      *property.Name
}

func NewAddWatcherEventChange(resource *property.Property) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *property.Property) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *property.Property) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *property.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *property.Property {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *property.Property {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *property.Property {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *property.Property {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[property.Name]*property.Property, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		property := currentChange.GetCurrent()
	...
	for _, change := range src.Changes {
		if change.IsDelete() {
			...
			property := change.GetCurrent()
			byName[*property.GetName()] = property
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, property := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(property))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[property.Name]*property.Property{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *property.Property
		switch item := change.ChangeType.(type) {
		case *property.PropertyChange_Added_:
			currentItem = item.Added.GetProperty()
		case *property.PropertyChange_Current_:
			currentItem = item.Current.GetProperty()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*property.PropertyChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *property.PropertyChange_Added_:
			newItem := item.Added.GetProperty()
		...
		case *property.PropertyChange_Modified_:
			updatedItem := item.Modified.GetProperty()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*property.PropertyChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range pendingChanges {
		name := *pendingChange.GetPropertyName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(property.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*property.Property_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*property.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(property_client.NewPropertyServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(property.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*property.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*property.ParentReference)
		}
		return params
	})
}
