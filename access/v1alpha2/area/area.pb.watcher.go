...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[area.Name]*area.Area
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*area.AreaChange
}

func NewWatcher(client area_client.AreaServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "area-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[area.Name]*area.Area{}
...
func NewAddWatcherEventChange(resource *area.Area) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *area.Area) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *area.Area) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *area.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *area.Area {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *area.Area {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *area.Area {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *area.Area {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[area.Name]*area.Area, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		area := currentChange.GetCurrent()
		byName[*area.GetName()] = area
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			area := change.GetCurrent()
			byName[*area.GetName()] = area
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, area := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(area))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*area.AreaChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *area.AreaChange_Added_:
			newItem := item.Added.GetArea()
		...
		case *area.AreaChange_Modified_:
			updatedItem := item.Modified.GetArea()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*area.AreaChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range pendingChanges {
		name := *pendingChange.GetAreaName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(area.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		return NewWatcher(area_client.NewAreaServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(area.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*area.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*area.ParentReference)
		}
		return params
	})
}
