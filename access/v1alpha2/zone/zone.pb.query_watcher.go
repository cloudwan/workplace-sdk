...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return zone.ZoneChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client zone_client.ZoneServiceClient,
...
func NewQueryWatcherWithIChan(id int, client zone_client.ZoneServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "zone-query-watcher").
	...
	for {
		stream, err := qw.client.WatchZones(ctx, &zone_client.WatchZonesRequest{
		...
		if err != nil {
			...
			pending := make([]*zone.ZoneChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetZoneChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(zone.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*zone.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*zone.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*zone.Filter)
		}
		return NewQueryWatcherWithIChan(id, zone_client.NewZoneServiceClient(cc), cfg, ch)
	})
}
