...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *zone.Zone_FieldMask
	OrderBy   *zone.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *zone.ParentReference
	Filter *zone.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[zone.Name]*zone.Zone
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*zone.ZoneChange
}

func NewWatcher(client zone_client.ZoneServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "zone-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[zone.Name]*zone.Zone{}
...
func NewAddWatcherEventChange(resource *zone.Zone) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *zone.Zone) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *zone.Zone) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *zone.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *zone.Zone {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *zone.Zone {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *zone.Zone {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *zone.Zone {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[zone.Name]*zone.Zone, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		zone := currentChange.GetCurrent()
		byName[*zone.GetName()] = zone
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			zone := change.GetCurrent()
			byName[*zone.GetName()] = zone
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, zone := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(zone))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*zone.ZoneChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *zone.ZoneChange_Added_:
			newItem := item.Added.GetZone()
		...
		case *zone.ZoneChange_Modified_:
			updatedItem := item.Modified.GetZone()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*zone.ZoneChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range pendingChanges {
		name := *pendingChange.GetZoneName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(zone.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*zone.Zone_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*zone.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(zone_client.NewZoneServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(zone.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*zone.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*zone.ParentReference)
		}
		return params
	})
}
