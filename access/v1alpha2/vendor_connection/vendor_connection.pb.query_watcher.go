...
type QueryWatcherParams struct {
	...
	Cursor       *vendor_connection.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*vendor_connection.VendorConnectionChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return vendor_connection.VendorConnectionChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client vendor_connection_client.VendorConnectionServiceClient,
...
func NewQueryWatcherWithIChan(id int, client vendor_connection_client.VendorConnectionServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "vendorConnection-query-watcher").
	...
	for {
		stream, err := qw.client.WatchVendorConnections(ctx, &vendor_connection_client.WatchVendorConnectionsRequest{
		...
		if err != nil {
			...
			pending := make([]*vendor_connection.VendorConnectionChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetVendorConnectionChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetVendorConnectionChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(vendor_connection.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*vendor_connection.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*vendor_connection.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*vendor_connection.Filter)
		}
		return NewQueryWatcherWithIChan(id, vendor_connection_client.NewVendorConnectionServiceClient(cc), cfg, ch)
	})
}
