...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *vendor_connection.VendorConnection_FieldMask
	OrderBy   *vendor_connection.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *vendor_connection.ParentReference
	Filter *vendor_connection.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[vendor_connection.Name]*vendor_connection.VendorConnection
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*vendor_connection.VendorConnectionChange
}

func NewWatcher(client vendor_connection_client.VendorConnectionServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &vendor_connection.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &vendor_connection.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "vendorConnection-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[vendor_connection.Name]*vendor_connection.VendorConnection{}
...
type WatcherEventChange struct {
	pre, post *vendor_connection.VendorConnection
	name      *vendor_connection.Name
}

func NewAddWatcherEventChange(resource *vendor_connection.VendorConnection) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *vendor_connection.VendorConnection) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *vendor_connection.VendorConnection) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *vendor_connection.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *vendor_connection.VendorConnection {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *vendor_connection.VendorConnection {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *vendor_connection.VendorConnection {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *vendor_connection.VendorConnection {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[vendor_connection.Name]*vendor_connection.VendorConnection, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		vendorConnection := currentChange.GetCurrent()
		byName[*vendorConnection.GetName()] = vendorConnection
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			vendorConnection := change.GetCurrent()
			byName[*vendorConnection.GetName()] = vendorConnection
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, vendorConnection := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(vendorConnection))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[vendor_connection.Name]*vendor_connection.VendorConnection{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *vendor_connection.VendorConnection
		switch item := change.ChangeType.(type) {
		case *vendor_connection.VendorConnectionChange_Added_:
			currentItem = item.Added.GetVendorConnection()
		case *vendor_connection.VendorConnectionChange_Current_:
			currentItem = item.Current.GetVendorConnection()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*vendor_connection.VendorConnectionChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *vendor_connection.VendorConnectionChange_Added_:
			newItem := item.Added.GetVendorConnection()
		...
		case *vendor_connection.VendorConnectionChange_Modified_:
			updatedItem := item.Modified.GetVendorConnection()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*vendor_connection.VendorConnectionChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetVendorConnectionName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(vendor_connection.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*vendor_connection.VendorConnection_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*vendor_connection.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(vendor_connection_client.NewVendorConnectionServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(vendor_connection.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*vendor_connection.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*vendor_connection.ParentReference)
		}
		return params
	})
}
