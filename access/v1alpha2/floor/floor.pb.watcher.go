...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *floor.Floor_FieldMask
	OrderBy   *floor.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *floor.ParentReference
	Filter *floor.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[floor.Name]*floor.Floor
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*floor.FloorChange
}

func NewWatcher(client floor_client.FloorServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &floor.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &floor.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "floor-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[floor.Name]*floor.Floor{}
...
type WatcherEventChange struct {
	pre, post *floor.Floor
	name      *floor.Name
}

func NewAddWatcherEventChange(resource *floor.Floor) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *floor.Floor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *floor.Floor) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *floor.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *floor.Floor {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *floor.Floor {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *floor.Floor {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *floor.Floor {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[floor.Name]*floor.Floor, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		floor := currentChange.GetCurrent()
		byName[*floor.GetName()] = floor
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			floor := change.GetCurrent()
			byName[*floor.GetName()] = floor
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, floor := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(floor))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*floor.FloorChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *floor.FloorChange_Added_:
			newItem := item.Added.GetFloor()
		...
		case *floor.FloorChange_Modified_:
			updatedItem := item.Modified.GetFloor()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*floor.FloorChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetFloorName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(floor.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*floor.Floor_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*floor.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(floor_client.NewFloorServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(floor.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*floor.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*floor.ParentReference)
		}
		return params
	})
}
