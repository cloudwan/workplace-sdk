...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return floor.FloorChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client floor_client.FloorServiceClient,
...
func NewQueryWatcherWithIChan(id int, client floor_client.FloorServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "floor-query-watcher").
	...
	for {
		stream, err := qw.client.WatchFloors(ctx, &floor_client.WatchFloorsRequest{
		...
		if err != nil {
			...
			pending := make([]*floor.FloorChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetFloorChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(floor.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*floor.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*floor.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*floor.Filter)
		}
		return NewQueryWatcherWithIChan(id, floor_client.NewFloorServiceClient(cc), cfg, ch)
	})
}
