...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *device_group.DeviceGroup_FieldMask
	OrderBy   *device_group.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *device_group.ParentReference
	Filter *device_group.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[device_group.Name]*device_group.DeviceGroup
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*device_group.DeviceGroupChange
}

func NewWatcher(client device_group_client.DeviceGroupServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &device_group.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &device_group.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "deviceGroup-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[device_group.Name]*device_group.DeviceGroup{}
...
type WatcherEventChange struct {
	pre, post *device_group.DeviceGroup
	name      *device_group.Name
}

func NewAddWatcherEventChange(resource *device_group.DeviceGroup) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *device_group.DeviceGroup) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *device_group.DeviceGroup) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *device_group.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *device_group.DeviceGroup {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *device_group.DeviceGroup {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *device_group.DeviceGroup {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *device_group.DeviceGroup {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[device_group.Name]*device_group.DeviceGroup, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		deviceGroup := currentChange.GetCurrent()
		byName[*deviceGroup.GetName()] = deviceGroup
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			deviceGroup := change.GetCurrent()
			byName[*deviceGroup.GetName()] = deviceGroup
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, deviceGroup := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(deviceGroup))
	}
}
...
func (qws *queryWatcherState) processSnapshot() []*WatcherEventChange {
	...
	prevCache := qws.cache
	qws.cache = map[device_group.Name]*device_group.DeviceGroup{}

	for _, change := range qws.pendingSnapshot {
		var currentItem *device_group.DeviceGroup
		switch item := change.ChangeType.(type) {
		case *device_group.DeviceGroupChange_Added_:
			currentItem = item.Added.GetDeviceGroup()
		case *device_group.DeviceGroupChange_Current_:
			currentItem = item.Current.GetDeviceGroup()
		}
...
func (qws *queryWatcherState) transform(rawChanges []*device_group.DeviceGroupChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *device_group.DeviceGroupChange_Added_:
			newItem := item.Added.GetDeviceGroup()
		...
		case *device_group.DeviceGroupChange_Modified_:
			updatedItem := item.Modified.GetDeviceGroup()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*device_group.DeviceGroupChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetDeviceGroupName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(device_group.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*device_group.DeviceGroup_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*device_group.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(device_group_client.NewDeviceGroupServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(device_group.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*device_group.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*device_group.ParentReference)
		}
		return params
	})
}
