...
type QueryWatcherParams struct {
	...
	Cursor       *device_group.PagerCursor
	ChunkSize    int
	PageSize     int
...
type QueryWatcherEvent struct {
	Identifier   int
	Changes      []*device_group.DeviceGroupChange
	Reset        bool
	LostSync     bool
	InSync       bool
	SnapshotSize int64
...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return device_group.DeviceGroupChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client device_group_client.DeviceGroupServiceClient,
...
func NewQueryWatcherWithIChan(id int, client device_group_client.DeviceGroupServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "deviceGroup-query-watcher").
	...
	for {
		stream, err := qw.client.WatchDeviceGroups(ctx, &device_group_client.WatchDeviceGroupsRequest{
		...
		if err != nil {
			...
			pending := make([]*device_group.DeviceGroupChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetDeviceGroupChanges()...)
						if resp.IsSoftReset {
						...
						outputEvt = &QueryWatcherEvent{
							Identifier:   qw.identifier,
							Changes:      resp.GetDeviceGroupChanges(),
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(device_group.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*device_group.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*device_group.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*device_group.Filter)
		}
		return NewQueryWatcherWithIChan(id, device_group_client.NewDeviceGroupServiceClient(cc), cfg, ch)
	})
}
