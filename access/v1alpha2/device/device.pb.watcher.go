...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[device.Name]*device.Device
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*device.DeviceChange
}
...
func (pw *Watcher) GetFilters() []*WatcherFilterParams {
	...
	for _, query := range pw.filters {
		...
		if query.Parent != nil {
			cQuery.Parent = &device.ParentReference{}
			*cQuery.Parent = *query.Parent
		}
		if query.Filter != nil {
			strValue, _ := query.Filter.ProtoString()
			cQuery.Filter = &device.Filter{}
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "device-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[device.Name]*device.Device{}
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[device.Name]*device.Device, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		device := currentChange.GetCurrent()
		byName[*device.GetName()] = device
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			device := change.GetCurrent()
			byName[*device.GetName()] = device
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, device := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(device))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*device.DeviceChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *device.DeviceChange_Added_:
			newItem := item.Added.GetDevice()
		...
		case *device.DeviceChange_Modified_:
			updatedItem := item.Modified.GetDevice()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*device.DeviceChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range append(qws.pendingSnapshot, pendingChanges...) {
		name := *pendingChange.GetDeviceName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(device.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*device.Device_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*device.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(device_client.NewDeviceServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(device.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*device.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*device.ParentReference)
		}
		return params
	})
}
