...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return site.SiteChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client site_client.SiteServiceClient,
...
func NewQueryWatcherWithIChan(id int, client site_client.SiteServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "site-query-watcher").
	...
	for {
		stream, err := qw.client.WatchSites(ctx, &site_client.WatchSitesRequest{
		...
		if err != nil {
			...
			pending := make([]*site.SiteChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetSiteChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(site.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*site.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*site.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*site.Filter)
		}
		return NewQueryWatcherWithIChan(id, site_client.NewSiteServiceClient(cc), cfg, ch)
	})
}
