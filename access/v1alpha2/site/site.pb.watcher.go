...
type WatcherConfig struct {
	*gotenaccess.WatcherConfigBase
	...
	View      view.View
	FieldMask *site.Site_FieldMask
	OrderBy   *site.OrderBy
	ChunkSize int
}

type WatcherFilterParams struct {
	Parent *site.ParentReference
	Filter *site.Filter
}
...
type queryWatcherState struct {
	...
	inSync          bool
	cache           map[site.Name]*site.Site
	filter          *WatcherFilterParams
	inSnapshot      bool
	pendingSnapshot []*site.SiteChange
}

func NewWatcher(client site_client.SiteServiceClient, config *WatcherConfig, filters ...*WatcherFilterParams) *Watcher {
...
func (pw *Watcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("watcher", "site-watcher")
...
func (pw *Watcher) onQueryLostSync(ctx context.Context, evt *QueryWatcherEvent) {
	...
	state.inSync = false
	state.cache = map[site.Name]*site.Site{}
...
func NewAddWatcherEventChange(resource *site.Site) *WatcherEventChange {
...
func NewModifyWatcherEventChange(current, previous *site.Site) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  previous,
		post: current,
...
func NewDeleteWatcherEventChange(deleted *site.Site) *WatcherEventChange {
	return &WatcherEventChange{
		pre:  deleted,
...
func (c *WatcherEventChange) GetName() *site.Name {
	return c.name
}
...
func (c *WatcherEventChange) GetAdded() *site.Site {
	if c.IsAdd() {
		return c.post
	}
	return nil
...
func (c *WatcherEventChange) GetDeleted() *site.Site {
	if c.IsDelete() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetPrevious() *site.Site {
	if c.IsModify() {
		return c.pre
	}
	return nil
...
func (c *WatcherEventChange) GetCurrent() *site.Site {
...
func (e *WatcherEvent) Merge(src *WatcherEvent) {
	...
	byName := make(map[site.Name]*site.Site, len(src.Changes)+len(e.Changes))
	for _, currentChange := range e.Changes {
		site := currentChange.GetCurrent()
		byName[*site.GetName()] = site
	}
	for _, change := range src.Changes {
		if change.IsDelete() {
			delete(byName, *change.GetName())
		} else {
			site := change.GetCurrent()
			byName[*site.GetName()] = site
		}
	}
	e.Changes = make([]*WatcherEventChange, 0, len(byName))
	for _, site := range byName {
		e.Changes = append(e.Changes, NewAddWatcherEventChange(site))
	}
}
...
func (qws *queryWatcherState) transform(rawChanges []*site.SiteChange) []*WatcherEventChange {
	watcherEventChanges := make([]*WatcherEventChange, 0, len(rawChanges))

	for _, change := range rawChanges {
		switch item := change.ChangeType.(type) {
		case *site.SiteChange_Added_:
			newItem := item.Added.GetSite()
		...
		case *site.SiteChange_Modified_:
			updatedItem := item.Modified.GetSite()
...
func (qws *queryWatcherState) computeSize(pendingChanges []*site.SiteChange) int64 {
	size := int64(len(qws.cache))
	for _, pendingChange := range pendingChanges {
		name := *pendingChange.GetSiteName()
...
func init() {
	gotenaccess.GetRegistry().RegisterWatcherConstructor(site.GetDescriptor(), func(cc grpc.ClientConnInterface, params *gotenaccess.WatcherConfigParams, filters ...gotenaccess.WatcherFilterParams) gotenaccess.Watcher {
		cfg := &WatcherConfig{
		...
		if params.FieldMask != nil {
			cfg.FieldMask = params.FieldMask.(*site.Site_FieldMask)
		}
		if params.OrderBy != nil {
			cfg.OrderBy = params.OrderBy.(*site.OrderBy)
		}
		typedFilters := make([]*WatcherFilterParams, 0, len(filters))
		for _, filter := range filters {
			typedFilters = append(typedFilters, filter.(*WatcherFilterParams))
		}
		return NewWatcher(site_client.NewSiteServiceClient(cc), cfg, typedFilters...)
	})
	gotenaccess.GetRegistry().RegisterWatcherFilterConstructor(site.GetDescriptor(), func(filter gotenresource.Filter, parent gotenresource.Reference) gotenaccess.WatcherFilterParams {
		params := &WatcherFilterParams{}
		if filter != nil {
			params.Filter = filter.(*site.Filter)
		}
		if parent != nil {
			params.Parent = parent.(*site.ParentReference)
		}
		return params
	})
}
