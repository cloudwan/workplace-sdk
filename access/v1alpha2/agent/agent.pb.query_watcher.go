...
func (e *QueryWatcherEvent) GetChanges() gotenresource.ResourceChangeList {
	return agent.AgentChangeList(e.Changes)
}
...
func NewQueryWatcher(id int, client agent_client.AgentServiceClient,
...
func NewQueryWatcherWithIChan(id int, client agent_client.AgentServiceClient,
...
func (qw *QueryWatcher) Run(ctx context.Context) error {
	log := ctxlogrus.Extract(ctx).
		WithField("query-watcher", "agent-query-watcher").
	...
	for {
		stream, err := qw.client.WatchAgents(ctx, &agent_client.WatchAgentsRequest{
		...
		if err != nil {
			...
			pending := make([]*agent.AgentChange, 0)
			for {
				...
				if err != nil {
					...
					if inSync {
						pending = append(pending, resp.GetAgentChanges()...)
						if resp.IsSoftReset {
...
func init() {
	gotenaccess.GetRegistry().RegisterQueryWatcherConstructor(agent.GetDescriptor(), func(id int, cc grpc.ClientConnInterface,
		...
		if params.Cursor != nil {
			cfg.Cursor = params.Cursor.(*agent.PagerCursor)
		}
		if params.Parent != nil {
			cfg.Parent = params.Parent.(*agent.ParentReference)
		}
		if params.Filter != nil {
			cfg.Filter = params.Filter.(*agent.Filter)
		}
		return NewQueryWatcherWithIChan(id, agent_client.NewAgentServiceClient(cc), cfg, ch)
	})
}
